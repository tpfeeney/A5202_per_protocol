...
## baseline variables; here we us the data set created with file long_dataset_creation_DR098.R 
## Here in this sensitivity analysis we censored when patients deviated from their assigned treatment
## for reasons other than toxicity. This addresses changes that occurred due to unblinding
##
## :::::::::::::::::::::::::::::::::::::::
...
trtmod<-read.csv(paste0(actg5202dir, "trtmod.csv"))
...
library(dplyr) 
#looking at who changed and why
trtmod %>%
  filter(changed2=="Yes") %>%
  filter(ModRs2 %in% c("Ran out of drug", "Med unblinded & switched","Study Participant/Guardian request")) %>%
  select(patid, rxcode, abc_tdf, efv_atv, lastdt2, changed2, ModRs2, i_chngwk2) %>%
  print(max=1500) %>% select(abc_tdf, ModRs2) %>% Hmisc::describe() 

trtmod %>%
  filter(changed2=="Yes") %>%
  filter(ModRs2 %in% c("Ran out of drug", "Med unblinded & switched","Study Participant/Guardian request")) %>%
  select(patid, rxcode, abc_tdf, efv_atv, lastdt2, changed2, ModRs2, i_chngwk2) %>%
  print(max=1500) %>% select(abc_tdf, ModRs2) %>% group_by(abc_tdf,ModRs2) 

cstbl<-trtmod %>% filter(changed2=="Yes") %>%
  filter(ModRs2 %in% c("Ran out of drug", "Med unblinded & switched","Study Participant/Guardian request"))
gmodels::CrossTable(cstbl$abc_tdf, cstbl$ModRs2)

#observations of people that switched
who_switched<-trtmod  %>% select(patid,ModRs2, changed2, i_chngwk2) %>% 
  filter(changed2=="Yes") %>% 
  filter(ModRs2 %in% c("Ran out of drug", "Med unblinded & switched","Study Participant/Guardian request")) %>% 
  mutate(chngwk=floor(i_chngwk2)) %>% 
  select(patid, ModRs2, chngwk) 

#Extracting IDs of those that switched
ids_who_switched<-trtmod %>% 
  filter(changed2=="Yes") %>% 
  filter(ModRs2 %in% c("Ran out of drug", "Med unblinded & switched","Study Participant/Guardian request")) %>% 
  mutate(chngwk=floor(i_chngwk2)) %>% pull(patid) 


#Combining data of those that switched with the main data set.
new_data<-df_tvary %>% left_join(who_switched %>% select(patid, ModRs2, chngwk), by=c("patid")) %>% 
  mutate(n_target_cens1=if_else(is.na(chngwk),target_cens_1, NA_real_),
         n_target_outcome1=if_else(is.na(chngwk),target_outcome_1,NA_real_))

#View(df1 %>% select(patid, Week, outcome, ever_outcome, virfail,
#                      virfail_wks, died, death_week, outcome_week,
#                      cens, ever_cens, cens_week_1, min, miss1,
#                      miss1_week, ever_dev1, d1_cens, d1_miss_time,
#                      ever_dev_doses_1, target_cens_1, target_outcome_1, 
#                    outcome))
...
## Make New DATA----
## :::::::::::::::::::::::::::::::::::::::
## Making data here that censors individuals when they switch their NRTI for non toxicity reasons 
## Prior to either being censored for LTFU or having outcome of interest.

allowed_weeks <- sort(unique(new_data$Week))

new_df <- new_data %>%
  group_by(patid) %>%
  mutate(
    wk_temp = first(na.omit(
      if_else(target_cens_1 == 1 | target_outcome_1 == 1, Week, NA_real_)
    )),
    chngwk_temp = {
      x <- chngwk[!is.na(chngwk)]
      if (length(x) == 0) NA_real_ else max(x)
    },
    chngwk_adj = case_when(
      is.na(chngwk_temp) ~ NA_real_,
      chngwk_temp %in% allowed_weeks ~ chngwk_temp,
      chngwk_temp < min(allowed_weeks) ~ NA_real_,
      TRUE ~ {
        valid_weeks <- allowed_weeks[allowed_weeks <= chngwk_temp[1]]
        if (length(valid_weeks) == 0) NA_real_ else max(valid_weeks)
      }
    ),
    marker = as.integer(
      any(chngwk < wk_temp, na.rm = TRUE)
    ),
    n_target_cens1 = case_when(
      marker == 1 & Week == chngwk_adj ~ 1,
      marker == 1 & Week > chngwk_adj ~ NA_real_,
      TRUE ~ target_cens_1
    ),
    n_target_outcome1 = case_when(
      marker == 1 & Week == chngwk_adj ~ 0,
      marker == 1 & Week > chngwk_adj ~ NA_real_,
      TRUE ~ target_outcome_1
    )
  ) %>% 
  ungroup()               

                    
View(new_df %>% filter(patid %in% ids_who_switched) %>% select(patid, Week, target_outcome_1,n_target_outcome1,
                                                                 target_cens_1,n_target_cens1, chngwk, wk_temp, marker))                
                    
View(new_df %>% filter(marker==1) %>% select(patid, Week, target_outcome_1,n_target_outcome1,
                                                               target_cens_1,n_target_cens1, chngwk, wk_temp, marker))   

View(new_df %>% filter(is.na(chngwk)) %>% select(patid, Week, target_outcome_1,n_target_outcome1,
                                             target_cens_1,n_target_cens1, chngwk, wk_temp, marker))
...
new_tvary_itt<-ggsurvfit::survfit2(Surv(time=Week, time2=end, event=outcome)~abc_tdf,
                                    data=new_df,
                                    robust=T,
...
new_itt_risks<-risk_xtract(new_tvary_itt, name="ITT", setype="B",
                            save=T, directory=data_dir,
                            filename="new_tvary_ittdf_DR098.csv"); print(new_itt_risks)
...
new_df<-wt_model_indic(data=new_df,
                        response="n_target_cens1",
                        exposure="abc_tdf",
...
weights_display(new_df,response="n_target_cens1",save=T,
                directory = results_tabs_dir, filename ='new_timevary_weights1_sum_DR098.tex' )
...
new_tvary_weighted1<-ggsurvfit::survfit2(Surv(time=Week, time2=end, event=n_target_outcome1)~abc_tdf,
                                          data=df_new,
                                          weights=n_target_cens1_wt,
                                          robust=T,
                                          id=patid)


new_tvary_week1_surv<-plot_survival_curves(list(new_tvary_itt,
                                                 new_tvary_weighted1),
                                            ylim=c(0,0.18),
                                            xlim=c(0,97),
                                            type="line",
                                            ...
                                            color_palette = c("ITT"= "black",
                                                              "1 Dose Weighted"="gray60"));print(new_tvary_week1_surv)

ggsave(paste0(results_figs_dir,"new_tvary_week1_surv.png"),
       plot=new_tvary_week1_surv, width=12, height=8, units='in')
...
df_flagged <- df %>%
  ...
  mutate(
    trigger =
      ...
      target_cens_1 == 0 &
      Week < chngwk &
      (is.na(lead(target_outcome_1)) | is.na(lead(target_cens_1)))
  ) %>%
  ungroup()
...
combined <- df_tvary %>%
  left_join(
    df_kept %>%
      select(patid, chngwk, ModRs2) %>%
      distinct(patid, .keep_all = TRUE),
    by = "patid"
  ) %>%
  group_by(patid) %>%
  mutate(
    event_week = case_when(
      is.na(chngwk)        ~ NA_real_,
      any(Week < chngwk)   ~ max(Week[Week < chngwk]),
      TRUE                 ~ NA_real_
    ),
    
    target_cens_1_n = case_when(
      is.na(chngwk)        ~ target_cens_1,
      Week < event_week    ~ 0,
      Week == event_week   ~ 1,
      Week > event_week    ~ NA_real_
    ),
    
    target_out_1_n = case_when(
      is.na(chngwk)        ~ target_outcome_1,
      Week < event_week    ~ 0,
      Week == event_week   ~ 1,
      Week > event_week    ~ NA_real_
    )
...
filter_patients_with_triggers <- function(
    df, 
    patid_col = "patid", 
    ids_to_keep, 
    week_col = "Week", 
    outcome_col = "target_outcome_1", 
    cens_col = "target_cens_1",
    chngwk_col = "chngwk"
) {
  
  # Convert column names to symbols
  patid_sym <- rlang::sym(patid_col)
  week_sym <- rlang::sym(week_col)
  outcome_sym <- rlang::sym(outcome_col)
  cens_sym <- rlang::sym(cens_col)
  chngwk_sym <- rlang::sym(chngwk_col)
  
  df %>%
    # Step 0: filter to selected IDs
    filter(!!patid_sym %in% ids_to_keep) %>%
    # Step 1: ensure numeric for ordering and comparisons
    mutate(
      !!week_sym := as.numeric(as.character(!!week_sym)),
      !!chngwk_sym := as.numeric(as.character(!!chngwk_sym))
    ) %>%
    arrange(!!patid_sym, !!week_sym) %>%
    # Step 2: compute trigger rows
    group_by(!!patid_sym) %>%
    mutate(
      trigger = 
        # 1) outcome=0 & cens=0 at Week=192
        (!!outcome_sym == 0 & !!cens_sym == 0 & !!week_sym == 192) |
        # 2) outcome=0 & cens=0 before NA & chngwk < Week
        (!!outcome_sym == 0 & !!cens_sym == 0 & 
           (is.na(lead(!!outcome_sym)) | is.na(lead(!!cens_sym))) & 
           !!week_sym > !!chngwk_sym) |
        # 3) outcome=1 OR cens=1 & chngwk < Week
        ((!!outcome_sym == 1 | !!cens_sym == 1) & !!week_sym > !!chngwk_sym)
    ) %>%
    # Step 3: keep all rows for patients with any trigger
    filter(any(trigger)) %>%
    ungroup() %>%
...
df_kept <- filter_patients_with_triggers(
  df = df_with_dividers,
  ids_to_keep = ids_who_switched
)
